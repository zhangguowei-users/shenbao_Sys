...
layui.define(['laytpl', 'layer'], function(exports) {
	...
	view.req = function(options) {
		...
		if(request.tokenName) {
			...
			//指定Bearer类型的Token
			options.headers.Authorization = 'Bearer ' + options.data[request.tokenName];
...
//		options.headers.Connection="keep-alive";
		...
		return $.ajax($.extend({
			...
			success: function(res) {
				...
				// }
				if(res[response.statusName] == statusCode.ok || res[response.statusName] == statusCode.ok1) {
					typeof options.done === 'function' && options.done(res);
				}
				...
				//其它异常
					...
					var error = [
						'<cite>请求异常，错误信息：</cite> ' + (res[response.msgName] || '返回状态码异常'), debug()
					].join('');
					view.error(error);
				}
			...
			error: function(e, code) {
				...
				if(e.status == 401) {
					error = [
						'请求异常，请重新登录系统<br><cite>错误信息：token信息无效或token已丢失</cite>', debug()
					].join('');
				view.error(error);
